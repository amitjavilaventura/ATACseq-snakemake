	...
	## Merging more than one replicate in the same peak calling step
		...
		rule peakAnnot:
			...
			output:
				...
				promoTargets      = "results/04_peakAnno/{{condition}}/{{condition}}-peaks_log{p_or_q}{pqvalue}_promoTargets.txt".format(pqvalue = pqval, p_or_q = pq),
				promoBed          = "results/04_peakAnno/{{condition}}/{{condition}}-peaks_log{p_or_q}{pqvalue}_promoPeaks.bed".format(pqvalue = pqval, p_or_q = pq),
				distalBed         = "results/04_peakAnno/{{condition}}/{{condition}}-peaks_log{p_or_q}{pqvalue}_distalPeaks.bed".format(pqvalue = pqval, p_or_q = pq),
			params:
				before = config["promoter"]["bTSS"],
				after  = config["promoter"]["aTSS"],
				genome = lambda wildcards: SAMPLES.GENOME[wildcards.sample]
